...
export class ModelEditModal extends Modal {
	...
	onOpen() {
		...
		new Setting(contentEl)
			...
			.addDropdown(dropdown => {
				...
				dropdown.setValue(this.model.providerId).onChange(value => {
	...
	private async refreshModels() {
		...
		try {
			...
			const providerIndex =
				this.plugin.settings.aiProviders.providers.findIndex(
					p => p.id === selectedProvider.id
				);
			if (providerIndex !== -1) {
				this.plugin.settings.aiProviders.providers[providerIndex] =
					selectedProvider;
	...
	private async fetchModelsFromProvider(
		...
		if (provider.type === "anthropic") {
			...
			try {
				const response = (await client.get("/v1/models")) as {
