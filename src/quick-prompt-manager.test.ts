...
describe('QuickPromptManager Integration Tests', () => {
	...
	beforeEach(() => {
		...
		mockSettings = {
			...
			aiProviders: {
				...
				models: [
					{ id: 'test-model', name: 'Test Model', providerId: 'provider1', modelName: 'test-model' }
				],
		...
		(ActionHandler as jest.MockedClass<typeof ActionHandler>).mockImplementation(() => mockActionHandler);
		(PromptProcessor as jest.MockedClass<typeof PromptProcessor>).mockImplementation(() => mockPromptProcessor);

		// Mock getAvailableModelsWithPluginAIProviders
		(getAvailableModelsWithPluginAIProviders as jest.MockedFunction<typeof getAvailableModelsWithPluginAIProviders>)
			.mockResolvedValue([
				{ id: 'test-model', name: 'Test Model', providerId: 'provider1', modelName: 'test-model' }
			]);
	...
	describe('processPrompt method refactoring', () => {
		it('should delegate to PromptProcessor with correct configuration', async () => {
			...
			await quickPromptManager['processPrompt'](userPrompt, modelId, outputMode);
		...
		it('should map outputMode "append" to APPEND_CURRENT location', async () => {
			...
			await quickPromptManager['processPrompt'](userPrompt, undefined, outputMode);
	...
	describe('behavior preservation verification', () => {
		...
		it('should preserve existing output mode logic', async () => {
			// Test replace mode
			await quickPromptManager['processPrompt']('test', undefined, 'replace');
			let callArgs = mockPromptProcessor.processPrompt.mock.calls[0][0];
			...
			// Test append mode
			await quickPromptManager['processPrompt']('test', undefined, 'append');
			callArgs = mockPromptProcessor.processPrompt.mock.calls[0][0];
		...
		it('should maintain all original quickPrompt settings except overridden ones', async () => {
			...
			expect(action.temperature).toBe(mockSettings.quickPrompt.temperature);
			expect(action.maxOutputTokens).toBe(mockSettings.quickPrompt.maxOutputTokens);
	...
	describe('error handling preservation', () => {
		it('should handle PromptProcessor errors gracefully', async () => {
			...
			await expect(quickPromptManager['processPrompt']('test')).rejects.toThrow('PromptProcessor failed');
		});
		...
		it('should handle ActionHandler errors gracefully', async () => {
			...
			await expect(quickPromptManager['processPrompt']('test')).rejects.toThrow('getTextInput failed');
		});
	});
	...
	describe('integration with existing UI management', () => {
		it('should not interfere with existing UI management methods', () => {
			...
			expect(typeof quickPromptManager['hideAllPromptBoxes']).toBe('function');
			expect(typeof quickPromptManager['positionPromptBox']).toBe('function');
			expect(typeof quickPromptManager['registerPromptBoxEvents']).toBe('function');
		});
